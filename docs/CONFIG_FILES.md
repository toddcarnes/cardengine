...
| `runouts` | `1` | Boards run at showdown, 1–3. `1` is classic poker; `2` runs it twice (each side pot splits across two boards dealt from the remaining shoe — all-in cash-game practice that cuts variance without changing anyone's equity). Spare boards are logged as `runout` lines. The shoe must cover every spare board up front (`seats × hole + board × runouts ≤ 52`) — a short shoe is rejected at load, never dealt half a runout. |
...
- `stud` mode requires exactly 7 hole + 0 board with `upcards = 4`, 2–8 seats, no straddle/kill/runouts (the standard full table is 8-max — street dealing plus the community-river fallback fit it in one deck).
...
- `seats × hole + board` must fit one 52-card deck (10-player Hold'em uses 25 cards, so there is plenty of room; stud is exempt — 8 × 7 = 56 exceeds the deck, but folds plus the community river keep it inside one shoe).
